...
def get_phase_data_file(phase_id):
    ...
    Checks (in order):
      1. Mixed file from a previous augment pass
      2. Pre-existing merged flat file (phase01_world_knowledge.jsonl)
      3. Subdirectory with multiple JSONL files (phase01/*.jsonl) → merges them
    """
    # 1. Already-augmented mixed file
    mixed = glob.glob(os.path.join(CURRICULUM_DIR, f'phase{phase_id:02d}_*_mixed.jsonl'))
    if mixed:
        return mixed[0]

    # 2. Flat file (generated by generate_curriculum_data.py)
    flat = glob.glob(os.path.join(CURRICULUM_DIR, f'phase{phase_id:02d}_*.jsonl'))
    flat = [f for f in flat if '_mixed' not in f]
    if flat:
        return flat[0]

    # 3. Subdirectory with per-topic JSONL files → merge into one flat file
...
def augment_with_replay(data_file, phase_id):
    ...
    # Replay from prior curriculum phases (check both flat files and subdirs)
    replay = []
    for prev_id in range(1, phase_id):
        ...
        if os.path.isdir(prev_subdir):
            prev_files += glob.glob(os.path.join(prev_subdir, '*.jsonl'))
