...
CURRICULUM = [
    ...
    (1, 1, "a", "Core Identity",               "phase01/1a_core_identity.jsonl",           800, 1.5e-5, False),
    (1, 1, "b", "Personality & Values",        "phase01/1b_personality.jsonl",             800, 1.3e-5, False),
    (1, 1, "c", "Emotional Intelligence",      "phase01/1c_emotional_intelligence.jsonl",  800, 1.1e-5, False),
    (1, 1, "d", "Identity Robustness",         "phase01/1d_identity_robustness.jsonl",     800, 9.0e-6, False),
    # Phase 2: Response Format & Basic Communication
    (1, 2, "a", "Direct Q&A",                  "phase02/2a_direct_qa.jsonl",               800, 1.4e-5, False),
    (1, 2, "b", "Output Format Control",       "phase02/2b_format.jsonl",                  800, 1.3e-5, False),
    (1, 2, "c", "Reading Comprehension",       "phase02/2c_reading.jsonl",                 800, 1.2e-5, False),
    (1, 2, "d", "Greetings & Social Phrases",  "phase02/2d_greetings.jsonl",               800, 1.1e-5, False),

    # ── Stage 2: Natural World & Human Knowledge ──────────────────────────────
    # Phase 3: Natural World / Science
    (2, 3, "a", "Physics & Chemistry",         "phase03/3a_physics_chemistry.jsonl",       800, 1.4e-5, False),
    (2, 3, "b", "Biology & Medicine",          "phase03/3b_biology_medicine.jsonl",        800, 1.3e-5, False),
    (2, 3, "c", "Astronomy & Earth Science",   "phase03/3c_astronomy_earth.jsonl",         800, 1.2e-5, False),
    (2, 3, "d", "Technology & Engineering",    "phase03/3d_technology.jsonl",              800, 1.1e-5, False),
    # Phase 4: Human World / History & Culture
    (2, 4, "a", "History & Politics",          "phase04/4a_history_politics.jsonl",        800, 1.3e-5, False),
    (2, 4, "b", "World Geography",             "phase04/4b_geography.jsonl",               800, 1.2e-5, False),
    (2, 4, "c", "Culture & Arts",              "phase04/4c_culture_arts.jsonl",            800, 1.1e-5, False),
    (2, 4, "d", "Economics & Daily Life",      "phase04/4d_economics_daily.jsonl",         800, 1.0e-5, False),

    # ── Stage 3: Reasoning Engine ─────────────────────────────────────────────
    # Phase 5: Math Reasoning
    (3, 5, "a", "Arithmetic",                  "phase05/5a_arithmetic.jsonl",              900, 1.4e-5, False),
    (3, 5, "b", "Word Problems",               "phase05/5b_word_problems.jsonl",           900, 1.3e-5, False),
    (3, 5, "c", "Percentages & Ratios",        "phase05/5c_percentages_ratios.jsonl",      900, 1.2e-5, False),
    (3, 5, "d", "Estimation & Mental Math",    "phase05/5d_estimation.jsonl",              900, 1.1e-5, False),
    # Phase 6: Logic & Abstract Reasoning
    (3, 6, "a", "Deductive Reasoning",         "phase06/6a_deduction.jsonl",               800, 1.2e-5, False),
    (3, 6, "b", "Analogies & Patterns",        "phase06/6b_analogies.jsonl",               800, 1.1e-5, False),
    (3, 6, "c", "Counterfactuals",             "phase06/6c_counterfactuals.jsonl",         800, 1.0e-5, False),
    (3, 6, "d", "Spatial & Temporal",          "phase06/6d_spatial_temporal.jsonl",        800, 9.0e-6, False),
    # Phase 7: Chain-of-Thought & Metacognition
    (3, 7, "a", "Step-by-Step Reasoning",      "phase07/7a_step_by_step.jsonl",            900, 1.1e-5, False),
    (3, 7, "b", "Problem Decomposition",       "phase07/7b_decomposition.jsonl",           900, 1.0e-5, False),
    (3, 7, "c", "Self-Verification",           "phase07/7c_verification.jsonl",            900, 9.0e-6, False),
    (3, 7, "d", "Honest Uncertainty",          "phase07/7d_uncertainty.jsonl",             900, 8.0e-6, False),

    # ── Stage 4: Instruction Following & Conversation ─────────────────────────
    # Phase 8: Precise Instruction Following
    (4, 8, "a", "Format & Length Control",     "phase08/8a_format_length.jsonl",           900, 1.2e-5, False),
    (4, 8, "b", "Task Execution",              "phase08/8b_task_execution.jsonl",          900, 1.1e-5, False),
    (4, 8, "c", "Compound Instructions",       "phase08/8c_compound.jsonl",                900, 1.0e-5, False),
    (4, 8, "d", "Persona & Register",          "phase08/8d_persona_register.jsonl",        800, 9.0e-6, False),
    # Phase 9: Conversational Intelligence
    (4, 9, "a", "Multi-Turn Coherence",        "phase09/9a_multi_turn.jsonl",              800, 1.0e-5, False),
    (4, 9, "b", "Topic Navigation",            "phase09/9b_topic_navigation.jsonl",        800, 9.0e-6, False),
    (4, 9, "c", "Ambiguity Handling",          "phase09/9c_ambiguity.jsonl",               800, 8.0e-6, False),
    (4, 9, "d", "Full Conversation",           "phase09/9d_full_conversation.jsonl",       800, 7.0e-6, False),

    # ── Stage 5: Kenyan & Swahili Identity ────────────────────────────────────
    # Phase 10: Kenya Deep Knowledge
    (5, 10, "a", "Kenya Geography",            "phase10/10a_geography.jsonl",              900, 1.2e-5, False),
    (5, 10, "b", "Kenya History",              "phase10/10b_history.jsonl",                900, 1.1e-5, False),
    (5, 10, "c", "Kenya Culture & Society",    "phase10/10c_culture.jsonl",                900, 1.0e-5, False),
    (5, 10, "d", "Kenya Economy & Tech",       "phase10/10d_economy.jsonl",                900, 9.0e-6, False),
    # Phase 11: Swahili Fluency
    (5, 11, "a", "Swahili Vocabulary",         "phase11/11a_vocabulary.jsonl",            1000, 1.2e-5, False),
    (5, 11, "b", "Swahili Grammar",            "phase11/11b_grammar.jsonl",               1000, 1.1e-5, False),
    (5, 11, "c", "Swahili Translation",        "phase11/11c_translation.jsonl",           1000, 1.0e-5, False),
    (5, 11, "d", "Swahili Conversation",       "phase11/11d_conversation.jsonl",          1000, 9.0e-6, False),

    # ── Stage 6: Technical Skills ─────────────────────────────────────────────
    # Phase 12: Code
    (6, 12, "a", "Code Reading",               "phase12/12a_code_reading.jsonl",           900, 1.1e-5, False),
    (6, 12, "b", "Code Writing",               "phase12/12b_code_writing.jsonl",           900, 1.0e-5, False),
    (6, 12, "c", "Code Debugging",             "phase12/12c_code_debugging.jsonl",         900, 9.0e-6, False),
    (6, 12, "d", "Code Explanation",           "phase12/12d_code_explanation.jsonl",       900, 8.0e-6, False),
    # Phase 13: Tool Use & Agency
    (6, 13, "a", "Tool Call Format",           "phase13/13a_tool_call_format.jsonl",       900, 1.1e-5, False),
    (6, 13, "b", "Tool Selection",             "phase13/13b_tool_selection.jsonl",         900, 1.0e-5, False),
    (6, 13, "c", "Multi-Step Agency",          "phase13/13c_multi_step_agency.jsonl",      900, 9.0e-6, False),
    (6, 13, "d", "Error Recovery",             "phase13/13d_error_recovery.jsonl",         900, 8.0e-6, False),
    # Phase 14: Structured Output & RAG
    (6, 14, "a", "JSON Output",                "phase14/14a_json_output.jsonl",            800, 1.0e-5, False),
    (6, 14, "b", "Markdown & Tables",          "phase14/14b_markdown_tables.jsonl",        800, 9.0e-6, False),
    (6, 14, "c", "RAG Context Grounding",      "phase14/14c_rag_synthesis.jsonl",          800, 8.0e-6, False),
    (6, 14, "d", "Long-Form Writing",          "phase14/14d_long_form.jsonl",              800, 7.0e-6, False),

    # ── Stage 7: Alignment & Values ───────────────────────────────────────────
    # Phase 15: Safety & Ethics
    (7, 15, "a", "Harm Refusals",              "phase15/15a_refusals.jsonl",               800, 9.0e-6, False),
    (7, 15, "b", "Honesty & Anti-Confab",      "phase15/15b_honesty.jsonl",               800, 8.0e-6, False),
    (7, 15, "c", "Injection Resistance",       "phase15/15c_injection_resistance.jsonl",   800, 7.0e-6, False),
    (7, 15, "d", "Ethical Reasoning",          "phase15/15d_ethics.jsonl",                800, 6.0e-6, False),
    # Phase 16: DPO Preference Alignment (always last — polishes existing capabilities)
    (7, 16, "a", "DPO Helpfulness",            "phase16/16a_dpo_helpfulness.jsonl",       1000, 5.0e-7, True),
    (7, 16, "b", "DPO Safety",                 "phase16/16b_dpo_safety.jsonl",            1000, 5.0e-7, True),
    (7, 16, "c", "DPO Honesty",                "phase16/16c_dpo_honesty.jsonl",           1000, 5.0e-7, True),
    (7, 16, "d", "DPO Style",                  "phase16/16d_dpo_style.jsonl",             1000, 5.0e-7, True),
]
